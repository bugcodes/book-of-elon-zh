---
layout: home
hero:
  name: 🚀 The Book of Elon
  text: 埃隆之书
  tagline: Elon Musk 最有用的想法，用他自己的话
  actions:
    - theme: brand
      text: 开始阅读
      link: /front-matter/01-notes.html
    - theme: alt
      text: GitHub 仓库
      link: https://github.com/bugcodes/book-of-elon-zh
features:
  - icon: 🎯
    title: 追求目标
    details: 过有目标的生活，像物理学家一样思考，理解工程的价值
  - icon: 💪
    title: 极限硬核工作
    details: 承担责任、打造卓越团队、疯狂紧迫感
  - icon: 🏢
    title: 创建公司
    details: Zip2、PayPal、Tesla、SpaceX 的创业历程
  - icon: 🌍
    title: 为了人类
    details: 可持续能源、AI 风险、多行星文明
---

## 📖 关于本书

这是 **Elon Musk** 的想法汇编，全部使用他自己的话语。通过收集他二十多年来的公开声明、采访和文章，尽可能清晰、直接地呈现他的思想。

### 🎯 本书结构

- **PART I** - 追求目标：生活目标、第一性原理思维、工程价值
- **PART II** - 极限硬核工作：责任、团队、组织、紧迫感
- **PART III** - 创建公司：Tesla 和 SpaceX 的完整故事
- **PART IV** - 为了人类：可持续能源、AI、多行星文明
- **附录** - 69 条核心马斯克方法、时间线、推荐书单

### 📊 翻译统计

| 项目 | 数据 |
|------|------|
| 总行数 | ~6,500+ 行 |
| 文件数 | 9 个 |
| 翻译风格 | 直译优先，中英对照 |
| 完成时间 | 2026-03-26 |

---

> "If you want the future to be good, you must make it so."
>
> **"如果你想要未来是好的，你必须让它如此。"**
>
> — Elon Musk

---

## 📱 关注公众号

**AI Tools Hub** - 发现最新 AI 工具，提升工作效率

<div style="text-align: center; margin: 30px 0;">
  <img src="/images/qrcode.jpg" alt="AI Tools Hub 公众号二维码" style="max-width: 300px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
</div>

**获取：**
- 🚀 最新 AI 工具推荐
- 💡 实用技巧与教程
- 📚 优质资源分享
- 🎁 独家优惠福利

**立即扫码关注，开启 AI 效率之旅！**

---
